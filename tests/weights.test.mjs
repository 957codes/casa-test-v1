...
test("nextActions: hard-demoting the top action removes it from the top", () => {
  const top = nextActions(INDEX, MEME, { completed: [], level: 0 })[0];
  const after = nextActions(INDEX, MEME, { completed: [], level: 0, weights: { byId: { [top.id]: 0.01 } } })[0];
...
test("nextActions: hard-promoting a low action makes it the recommendation", () => {
  ...
  const promoted = nextActions(INDEX, MEME, { completed: [], level: 0, weights: { byId: { [low.id]: 100 } } })[0];
